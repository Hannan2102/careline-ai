"""EHRProvider behaviour, exercised against the in-memory provider.

These assertions are written to be reusable: the HAPI-backed provider must
satisfy exactly the same contract (tests/integration/test_local_fhir_provider.py).
"""
...
class TestPatientSearch:
    async def test_exact_match_returns_one_patient(self, ehr: MemoryFHIRProvider) -> None:
        matches = await ehr.search_patients("John Smith", JOHN_SMITH_DOB)
        assert [p.reference for p in matches] == [JOHN_SMITH]

    async def test_unknown_patient_returns_nothing(self, ehr: MemoryFHIRProvider) -> None:
        assert await ehr.search_patients("Jane Doe", date(1970, 1, 1)) == []

    async def test_wrong_dob_does_not_match(self, ehr: MemoryFHIRProvider) -> None:
        assert await ehr.search_patients("John Smith", date(1985, 2, 16)) == []

    async def test_duplicate_name_and_dob_returns_both_candidates(
        self, ehr: MemoryFHIRProvider
    ) -> None:
    ...
    async def test_search_is_case_and_whitespace_insensitive(self, ehr: MemoryFHIRProvider) -> None:
        matches = await ehr.search_patients("  john   SMITH ", JOHN_SMITH_DOB)
...
class TestBooking:
    ...
    async def test_appointment_type_determines_duration(self, ehr: MemoryFHIRProvider) -> None:
    ...
    async def test_double_booking_the_same_slot_is_refused(self, ehr: MemoryFHIRProvider) -> None:
    ...
    async def test_patient_cannot_hold_two_overlapping_appointments(
        self, ehr: MemoryFHIRProvider
    ) -> None:
        slots = await ehr.get_available_slots(
    ...
    async def test_booking_an_unknown_slot_raises(self, ehr: MemoryFHIRProvider) -> None:
    ...
    async def test_booking_an_unknown_patient_raises(self, ehr: MemoryFHIRProvider) -> None:
        slot = await first_free_slot(ehr)
...
class TestCancellation:
    async def test_cancelling_releases_the_slot(self, ehr: MemoryFHIRProvider) -> None:
    ...
    async def test_cancelled_appointments_leave_the_booked_list(
        self, ehr: MemoryFHIRProvider
    ) -> None:
    ...
    async def test_cancelling_twice_is_idempotent(self, ehr: MemoryFHIRProvider) -> None:
        booked = await ehr.get_appointments(JOHN_SMITH)
    ...
    async def test_cancelling_an_unknown_appointment_raises(self, ehr: MemoryFHIRProvider) -> None:
...
class TestRescheduling:
    ...
    async def test_failed_reschedule_leaves_the_original_intact(
        self, ehr: MemoryFHIRProvider
    ) -> None:
    ...
    async def test_cancelled_appointments_cannot_be_rescheduled(
        self, ehr: MemoryFHIRProvider
    ) -> None:
        original = (await ehr.get_appointments(JOHN_SMITH))[0]
...
class TestAvailability:
    async def test_no_slots_are_offered_at_weekends(self, ehr: MemoryFHIRProvider) -> None:
        slots = await ehr.get_available_slots(
    ...
    async def test_offered_slots_carry_the_requested_duration(
        self, ehr: MemoryFHIRProvider
    ) -> None:
        slots = await ehr.get_available_slots(
    ...
    async def test_unknown_practitioner_raises(self, ehr: MemoryFHIRProvider) -> None:
    ...
    async def test_the_seeded_demo_appointment_blocks_its_own_time(
        self, ehr: MemoryFHIRProvider
    ) -> None:
...
class TestMedications:
    ...
    async def test_medication_lookup_by_name_is_case_insensitive(
        self, ehr: MemoryFHIRProvider
    ) -> None:
    ...
    async def test_unknown_medication_returns_none(self, ehr: MemoryFHIRProvider) -> None:
        assert await ehr.get_medication_request(JOHN_SMITH, "amoxicillin") is None

    async def test_a_prescription_without_dosage_text_reports_none(
        self, ehr: MemoryFHIRProvider
    ) -> None:
    ...
    async def test_medications_are_scoped_to_the_patient(self, ehr: MemoryFHIRProvider) -> None:
...
class TestClinicalRecords:
    async def test_conditions_and_allergies_are_returned(self, ehr: MemoryFHIRProvider) -> None:
...
class TestPatientCreation:
    async def test_created_patients_are_searchable(self, ehr: MemoryFHIRProvider) -> None:
    ...
    async def test_new_patients_can_book_immediately(self, ehr: MemoryFHIRProvider) -> None:
